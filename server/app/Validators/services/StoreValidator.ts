...
export default class StoreValidator {
	...
	public schema = schema.create({
		translation_id: schema.number([rules.exists({ column: 'id', table: 'translations' })]),
		frequency_id: schema.number([rules.exists({ column: 'id', table: 'frequencies' })]),
		is_visible: schema.boolean(),
		price: schema.string({ trim: true }, [rules.regex(/^[0-9]{0,}/g)])
	})
