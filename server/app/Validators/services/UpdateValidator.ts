...
export default class StoreValidator {
	...
	public schema = schema.create({
		translation_id: schema.number.optional([rules.exists({ column: 'id', table: 'translations' })]),
		frequency_id: schema.number.optional([rules.exists({ column: 'id', table: 'frequencies' })]),
		is_visible: schema.boolean.optional(),
		price: schema.string.optional({ trim: true }, [rules.regex(/^[0-9]{0,}/g)])
	})
	...
	public messages = {
		...
		'translation_id.required': 'Le code de traduction est obligatoire',
		'price.string': 'Le prix doit-être une chaine de caractère',
		'price.regex': 'Le prix doit-être compris entre 0 et 9 répétable'
	}
}
